...
\lecture{4}{Notes \#4: Symmetric Encryption and Security Reductions}
...
Let's review where we are thus far. We started with ciphers and perfect
secrecy, and proved the one-time pad (OTP) was perfectly secret. We then
identified three limitations of the OTP that limit its practicality: (1) Long
keys, (2) Security only for one-time encryption, and (3) Malleability.  We
began addressing the first limitation by developing pseudorandom generators and
constructing a ``pseudo-OTP.''

Our agenda now is analyze how the security of the pseudo-OTP relates to the
underlying pseudorandom generator $G$. Intuitively, we expect that a good $G$
can stand in for a truly random pad, but we'd like to make that statement
rigorous. In order to do this, we'll need to take a step back and define what
\emph{security} even means for thing we built. With that definition, we'll then
be able to carry out a rigorous version of the statement ``If $G$ is a good
PRG, then the pseudo-OTP with $G$ is a secure way to encrypt.'' The steps in
doing this are themselves pretty short to write out, but also involve some
interesting and novel concepts from theoretical computer science.  Since we're
going to develop other definitions and reuse the proof technique later, we do
this carefully.
...
%We start by defining the \emph{syntax} (i.e.  input/output behavior) of what
%exactly we want separately from the \emph{security} we want.  Afterwards we
%give a security definition, and finally analyze the pseudo-OTP.
%
